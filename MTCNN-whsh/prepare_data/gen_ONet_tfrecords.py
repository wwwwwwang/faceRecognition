...
def _get_output_filename(output_dir, type, net):
    ...
    return '%s/%s_landmark.tfrecord' % (output_dir, type)
    

def run(dataset_dir, net, type, output_dir, name='MTCNN', shuffling=False):
    ...
    #tfrecord name 
    tf_filename = _get_output_filename(output_dir, type, net)
    if tf.gfile.Exists(tf_filename):
        print('Dataset files already exist. Exiting without re-creating them.')
        return
    # GET Dataset, and shuffling.
    dataset = get_dataset(dataset_dir, type, net=net)
...
if __name__ == '__main__':
    ...
    type = 'part' # pos part neg landmark
    run(dir, net, type, output_directory, shuffling=True)
